...
for i, batch in enumerate(im_batches):
    ...
    if CUDA:
        batch = batch.cuda()
    client_process(model, batch, split_layer, required_layers)
    prediction = server_process(model, split_layer, confidence, num_classes, nms_thesh)
    end = time.time()
...
def write(x, results):
    # c1 = tuple(x[1:3].int())
    # c2 = tuple(x[3:5].int())
    c1 = (int(x[1]), int(x[2]))  # 좌상단 좌표
    c2 = (int(x[3]), int(x[4]))  # 우하단 좌표
    img = results[int(x[0])]
    cls = int(x[-1])
