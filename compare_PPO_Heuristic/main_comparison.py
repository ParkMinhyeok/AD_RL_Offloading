...
def run_ppo_experiment(device):
    ...
    for e in range(config.NUM_EPISODES):
        ...
        if e % 10 == 0 and e > 0:
            ...
            print(f"  에피소드 {e}/{config.NUM_EPISODES} | 지난 10 에피소드 평균 점수: {avg_score:.2f}")
    
    # 학습 후 최종 성능 평가
    print("  학습 완료. 최종 성능 평가 진행...")
    set_seed(config.SEED) # 동일한 환경에서 평가하기 위해 시드 재설정
    state = env.reset()
    done = False
    while not done:
        prob = agent.pi(torch.from_numpy(np.array(state)).float().to(device))
        action = torch.argmax(prob).item() # 가장 확률이 높은 행동 선택
